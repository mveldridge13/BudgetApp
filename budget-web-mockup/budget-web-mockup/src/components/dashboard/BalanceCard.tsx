...
export default function BalanceCard({
  ...
  const getProgressBarColor = () => {
    ...
    return '#10B981'; // success green from mobile app
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-6 transition-all duration-200 hover:shadow-lg" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.04)' }}>
      ...
      <div className="mb-6">
        ...
        <p className="text-4xl font-bold text-gray-900 tracking-tight">{formatCurrency(balance)}</p>
      </div>

      {/* Left to Spend and Total Expenses */}
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl p-5" style={{ backgroundColor: '#EEF2FF' }}>
          <p className="text-xs font-medium text-gray-500 mb-2">Left to Spend</p>
          <p className="text-2xl font-bold mb-4 tracking-tight" style={{ color: '#6366f1' }}>{formatCurrency(leftToSpend)}</p>
        ...
        <div className="rounded-xl p-5" style={{ backgroundColor: '#FEF2F2' }}>
          ...
          <p className="text-2xl font-bold tracking-tight" style={{ color: '#F87171' }}>{formatCurrency(totalExpenses)}</p>

          {/* Committed vs Discretionary Breakdown */}
          <div className="mt-4 pt-4 space-y-2.5" style={{ borderTop: '1px solid rgba(254, 202, 202, 0.5)' }}>
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                <div className="w-2 h-2 rounded-full mr-2.5" style={{ backgroundColor: '#F59E0B' }}></div>
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                <div className="w-2 h-2 rounded-full mr-2.5" style={{ backgroundColor: '#6366f1' }}></div>
                <span className="text-xs text-gray-500 font-medium">Discretionary</span>
              </div>
              <span className="text-xs font-semibold text-gray-900">
