...
export default function AnalyticsPage() {
  ...
  const [billsAnalytics, setBillsAnalytics] = useState<BillsAnalyticsResponse | null>(null);
  const [payPeriodStatus, setPayPeriodStatus] = useState<string | null>(null);

  // Modal state
  const [showDiscretionaryBreakdown, setShowDiscretionaryBreakdown] = useState(false);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [analyticsData, incomeData, billsData, incomeInfo] = await Promise.all([
  ...
  const totalDiscretionaryExpenses = analytics?.monthlyTrends?.reduce(
  ...
  const chartData = analytics?.monthlyTrends?.map((trend, index) => {
    ...
    if (selectedPeriod === 'monthly') {
      ...
      label = date.toLocaleDateString('en-AU', { weekday: 'short', day: 'numeric' });
    }
  ...
  const statistics = {
    ...
    highestSpendingPeriod: chartData.length > 0
      ? chartData.reduce((max, curr) => curr.expenses > max.expenses ? curr : max)
      : null,
    highestDiscretionaryPeriod: chartData.length > 0
      ? chartData.reduce((max, curr) => curr.discretionary > max.discretionary ? curr : max)
      : null,
  };
    ...
    <div className="space-y-6">
      ...
      {selectedTab === 'spending' && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-500">
                {selectedPeriod.charAt(0).toUpperCase() + selectedPeriod.slice(1)} Average
              </p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-500">vs Previous Period</p>
              <p className={`text-2xl font-bold mt-1 ${
                statistics.percentageChange >= 0 ? 'text-red-600' : 'text-green-600'
              }`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-500">Discretionary Average</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
                {formatCurrency(statistics.averageDiscretionary)}
              </p>
            </div>
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-500">Discretionary vs Previous</p>
              <p className={`text-2xl font-bold mt-1 ${
                statistics.discretionaryPercentageChange >= 0 ? 'text-red-600' : 'text-green-600'
              }`}>
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-2">
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: category.categoryColor || '#6366F1'
                        }}
                      />
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {statistics.highestSpendingPeriod && (
                <div className="p-3 bg-gray-50 rounded-lg border-l-4 border-l-indigo-500">
                  <p className="text-sm text-gray-700">
                    <span className="font-semibold">Highest total spending:</span>{' '}
              ...
              {statistics.highestDiscretionaryPeriod && statistics.highestDiscretionaryPeriod.discretionary > 0 && (
                <div
                  className="p-3 bg-gray-50 rounded-lg border-l-4 border-l-green-500 cursor-pointer hover:bg-gray-100 transition-colors"
                  onClick={() => setShowDiscretionaryBreakdown(true)}
                >
                  <p className="text-sm text-gray-700">
                    <span className="font-semibold">Highest discretionary spending:</span>{' '}
                    {statistics.highestDiscretionaryPeriod.date} with{' '}
                    {formatCurrency(statistics.highestDiscretionaryPeriod.discretionary)}
                  </p>
                  <p className="text-xs text-indigo-600 mt-1 italic">Click for details</p>
                </div>
              )}

              <div className={`p-3 bg-gray-50 rounded-lg border-l-4 ${
                statistics.percentageChange >= 0 ? 'border-l-red-500' : 'border-l-green-500'
              }`}>
                <p className="text-sm text-gray-700">
                  <span className="font-semibold">Total spending trend:</span>{' '}
                  You&apos;re spending {Math.abs(statistics.percentageChange).toFixed(1)}%{' '}
                  {statistics.percentageChange >= 0 ? 'more' : 'less'} than last period
                </p>
              </div>
              ...
              {analytics?.spendingVelocity && (
                <div
                  className={`p-3 bg-gray-50 rounded-lg border-l-4 ${
                    getVelocityStatusBorder(analytics.spendingVelocity.velocityStatus)
                  } cursor-pointer hover:bg-gray-100 transition-colors`}
                  onClick={() => setShowVelocityBreakdown(true)}
                >
                  <p className="text-sm text-gray-700">
                    <span className="font-semibold">Spending Velocity:</span>{' '}
                    <span className={getVelocityStatusColor(analytics.spendingVelocity.velocityStatus)}>
                      {analytics.spendingVelocity.velocityStatus.replace('_', ' ')}
                    </span>
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    Daily average: {formatCurrency(analytics.spendingVelocity.dailyAverage)} |
                    Recommended: {formatCurrency(analytics.spendingVelocity.recommendedDailySpending)}
                  </p>
                  <p className="text-xs text-indigo-600 mt-1 italic">Click for details</p>
                </div>
              )}
      ...
      {selectedTab === 'income' && (
        <>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              ...
              {incomeAnalytics?.monthChangePercentage !== undefined && (
                <p className={`text-sm mt-1 ${
                  incomeAnalytics.monthChangePercentage >= 0 ? 'text-green-600' : 'text-red-600'
                }`}>
                  {incomeAnalytics.monthChangePercentage >= 0 ? '+' : ''}
                  {incomeAnalytics.monthChangePercentage.toFixed(1)}% from last month
                </p>
              )}
            </div>
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-500">This Pay Period</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            {incomeAnalytics?.incomeBySource && incomeAnalytics.incomeBySource.length > 0 ? (
              <div className="space-y-4">
              ...
              <p className="text-gray-500 text-sm">No income sources found for this month</p>
            )}
          </div>

          {/* Recurring vs Ad-hoc */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Recurring vs Ad-hoc Income</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="border border-gray-200 rounded-lg p-4 text-center">
                <p className="text-sm font-medium text-gray-500 mb-2">Recurring</p>
                <p className="text-xl font-bold text-gray-900">
                  {formatCurrency(incomeAnalytics?.incomeBreakdown?.recurring?.amount || 0)}
                </p>
                <p className="text-sm text-indigo-600 mt-1">
                  {incomeAnalytics?.incomeBreakdown?.recurring?.percentage?.toFixed(0) || 0}%
                </p>
              </div>
              <div className="border border-gray-200 rounded-lg p-4 text-center">
                <p className="text-sm font-medium text-gray-500 mb-2">Ad-hoc</p>
                <p className="text-xl font-bold text-gray-900">
                  {formatCurrency(incomeAnalytics?.incomeBreakdown?.adhoc?.amount || 0)}
                </p>
                <p className="text-sm text-indigo-600 mt-1">
                  {incomeAnalytics?.incomeBreakdown?.adhoc?.percentage?.toFixed(0) || 0}%
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Recent Income Entries</h3>
            {incomeAnalytics?.recentIncomeEntries && incomeAnalytics.recentIncomeEntries.length > 0 ? (
              <div className="space-y-3">
              ...
              <p className="text-gray-500 text-sm">No recent income entries found</p>
            )}
          </div>
              ...
              {incomeAnalytics?.insights ? (
                <>
                  <div className="p-3 bg-gray-50 rounded-lg border-l-4 border-l-green-500">
                    <p className="text-sm text-gray-700">
                      ...
                      Your income consistency score is {incomeAnalytics.insights.consistencyScore}%
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg border-l-4 border-l-indigo-500">
                    <p className="text-sm text-gray-700">
                      <span className="font-semibold">Income trend:</span>{' '}
                      Your income is{' '}
                ...
                <div className="p-3 bg-gray-50 rounded-lg border-l-4 border-l-indigo-500">
                  <p className="text-sm text-gray-700">
                    ...
                    Add income transactions to see detailed insights about your income patterns and trends.
                  </p>
                </div>
              )}
            </div>
      ...
      {selectedTab === 'bills' && (
        <>
            ...
            <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-700">Bills filtered by your pay period</p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-500">Total Bills This Pay Period</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              ...
              <p className="text-sm text-gray-500 mt-1">
                {billsAnalytics?.paidBills || 0} of {billsAnalytics?.totalBills || 0} bills
              </p>
            </div>
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-500">
              ...
              <p className={`text-2xl font-bold mt-1 ${
                (billsAnalytics?.overdueBills || 0) > 0 ? 'text-red-600' : 'text-yellow-600'
              }`}>
                {formatCurrency(
          ...
          {/* Bills Progress */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Bills Progress</h3>
            <div className="text-center space-y-3">
              <p className="text-sm text-gray-600">
                {billsAnalytics?.paidBills || 0} of {billsAnalytics?.totalBills || 0} bills paid • {' '}
                {formatCurrency(billsAnalytics?.paidAmount || 0)} of {formatCurrency(billsAnalytics?.totalAmount || 0)} settled
              </p>
          ...
          {/* Upcoming Bills */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Upcoming Bills</h3>
            <p className="text-sm text-gray-500 mb-4">Due within the next 7 days</p>
            {billsAnalytics?.upcomingBills && billsAnalytics.upcomingBills.length > 0 ? (
              <div className="space-y-3">
                {billsAnalytics.upcomingBills.map((bill, index) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{bill.category?.name || 'General'}</p>
                      <p className="text-xs text-yellow-600 mt-1">{bill.dueDate ? formatDueDate(bill.dueDate) : 'No due date'}</p>
                    </div>
          ...
          {/* Paid Bills */}
          {billsAnalytics?.paidBillsList && billsAnalytics.paidBillsList.length > 0 && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Paid Bills</h3>
              <div className="space-y-3">
                {billsAnalytics.paidBillsList.map((bill, index) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{bill.category?.name || 'General'}</p>
                      <p className="text-xs text-gray-500 mt-1">
                        Paid: {bill.dueDate ? new Date(bill.dueDate).toLocaleDateString('en-AU', {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric',
                        }) : 'Unknown'}
          ...
          {/* Overdue Bills */}
          {billsAnalytics?.overdueBillsList && billsAnalytics.overdueBillsList.length > 0 && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Overdue Bills</h3>
              <div className="space-y-3">
                {billsAnalytics.overdueBillsList.map((bill, index) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{bill.category?.name || 'General'}</p>
                      <p className="text-xs text-red-600 mt-1">{bill.dueDate ? formatDueDate(bill.dueDate) : 'No due date'}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-gray-900">
                        {formatCurrency(Math.abs(bill.amount))}
          ...
          {/* Bills Insights */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Bills Insights</h3>
            <div className="space-y-3">
              {billsAnalytics?.upcomingBills && billsAnalytics.upcomingBills.length > 0 && (
                <div className="p-3 bg-gray-50 rounded-lg border-l-4 border-l-yellow-500">
                  <p className="text-sm text-gray-700">
                    ...
                    {billsAnalytics.upcomingBills.length === 1 ? 'upcoming bill' : 'upcoming bills'}{' '}
                    totaling {formatCurrency(
                      billsAnalytics.upcomingBills.reduce((sum, bill) => sum + Math.abs(bill.amount), 0)
                    )}.
                  </p>
                </div>
              )}
              ...
              {(billsAnalytics?.totalBills || 0) > 0 && (
                <div className={`p-3 bg-gray-50 rounded-lg border-l-4 ${
                  (billsAnalytics?.progress || 0) >= 50 ? 'border-l-green-500' : 'border-l-yellow-500'
                }`}>
                  <p className="text-sm text-gray-700">
                    ...
                    You&apos;ve paid {billsAnalytics?.progress || 0}% of your monthly bills.
                    {(billsAnalytics?.progress || 0) >= 80
              ...
              {(!billsAnalytics?.totalBills || billsAnalytics.totalBills === 0) && (
                <div className="p-3 bg-gray-50 rounded-lg border-l-4 border-l-indigo-500">
                  <p className="text-sm text-gray-700">
                    <span className="font-semibold">Get started:</span>{' '}
                    Add your recurring bills and expenses with due dates to track them here.
                    This helps you stay on top of your financial obligations.
                  </p>
                </div>
              )}
            </div>
      ...
      <Modal
        ...
        onClose={() => setShowDiscretionaryBreakdown(false)}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-indigo-50 rounded-lg p-4 text-center">
            <p className="text-sm text-indigo-600 font-medium mb-1">
              {statistics.highestDiscretionaryPeriod?.date || 'This Period'}
            </p>
            <p className="text-3xl font-bold text-indigo-700">
              {formatCurrency(statistics.highestDiscretionaryPeriod?.discretionary || 0)}
            </p>
            <p className="text-sm text-indigo-600 mt-1">in discretionary spending</p>
          </div>
          ...
          {analytics?.categoryBreakdown && analytics.categoryBreakdown.length > 0 && (
            <div>
              ...
              <div className="flex justify-center mb-6">
                <div className="relative w-48 h-48">
                  <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                    {(() => {
                      const total = analytics.categoryBreakdown.reduce((sum, cat) => sum + cat.amount, 0);
                      let currentAngle = 0;
                      return analytics.categoryBreakdown.slice(0, 6).map((category) => {
                        ...
                        return (
                          <circle
                            ...
                            r="40"
                            fill="none"
                            stroke={category.categoryColor || '#6366F1'}
                            strokeWidth="20"
              ...
              {/* Category List */}
              <div className="space-y-3">
                {analytics.categoryBreakdown.slice(0, 8).map((category) => (
              ...
              {analytics?.categoryBreakdown && analytics.categoryBreakdown.length > 0 && (
                <div className="p-3 bg-amber-50 rounded-lg border-l-4 border-l-amber-500">
                  <p className="text-sm text-gray-700">
                    ...
                    {analytics.categoryBreakdown[0].categoryName} accounts for{' '}
                    {analytics.categoryBreakdown[0].percentage.toFixed(0)}% of your spending
                  </p>
                </div>
              )}
              {statistics.percentageChange !== 0 && (
                <div className={`p-3 rounded-lg border-l-4 ${
                  statistics.percentageChange > 0 ? 'bg-red-50 border-l-red-500' : 'bg-green-50 border-l-green-500'
                }`}>
                  <p className="text-sm text-gray-700">
                    <span className="font-semibold">Trend:</span>{' '}
                    Spending is {Math.abs(statistics.percentageChange).toFixed(1)}%{' '}
                    {statistics.percentageChange > 0 ? 'higher' : 'lower'} than the previous period
                  </p>
                </div>
              )}
            </div>
            ...
            <>
              ...
              <div className="flex justify-center">
                <span className={`px-4 py-2 rounded-full text-sm font-semibold ${
                  analytics.spendingVelocity.velocityStatus === 'ON_TRACK'
                    ...
                    : analytics.spendingVelocity.velocityStatus === 'SLIGHTLY_HIGH'
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-lg p-4 text-center">
                  <p className="text-xs text-gray-500 mb-1">Current Month Spent</p>
                  <p className="text-xl font-bold text-gray-900">
                    {formatCurrency(analytics.spendingVelocity.currentMonthSpent)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4 text-center">
                  <p className="text-xs text-gray-500 mb-1">Projected Monthly</p>
                  <p className="text-xl font-bold text-indigo-600">
                    {formatCurrency(analytics.spendingVelocity.projectedMonthlySpending)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4 text-center">
                ...
                <div className="bg-green-50 rounded-lg p-4 text-center">
                  <p className="text-xs text-green-600 mb-1">Recommended Daily</p>
                  <p className="text-xl font-bold text-green-700">
                    {formatCurrency(analytics.spendingVelocity.recommendedDailySpending)}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div
                    ...
                    style={{
                      width: `${(analytics.spendingVelocity.daysElapsed / analytics.spendingVelocity.daysInMonth) * 100}%`
                    }}
                  />
                </div>
                <p className="text-xs text-gray-500 mt-1 text-center">
                  {analytics.spendingVelocity.daysInMonth - analytics.spendingVelocity.daysElapsed} days remaining this month
                </p>
              </div>
              ...
              {analytics.dailyBurnRate && (
                <div>
                  <h4 className="text-sm font-semibold text-gray-900 mb-3">Daily Burn Rate</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 rounded-lg p-3">
                      <p className="text-xs text-gray-500">Current Rate</p>
                      <p className="text-lg font-bold text-gray-900">
                        {formatCurrency(analytics.dailyBurnRate.currentDailyBurnRate)}/day
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3">
                      <p className="text-xs text-gray-500">Sustainable Rate</p>
                      <p className="text-lg font-bold text-green-600">
                        {formatCurrency(analytics.dailyBurnRate.sustainableDailyRate)}/day
                      </p>
                    </div>
                  </div>
                  {analytics.dailyBurnRate.daysUntilBudgetExceeded !== null && (
                    <div className={`mt-3 p-3 rounded-lg ${
                      analytics.dailyBurnRate.daysUntilBudgetExceeded <= 7
                        ...
                        : analytics.dailyBurnRate.daysUntilBudgetExceeded <= 14
                        ? 'bg-yellow-50 border border-yellow-200'
                        : 'bg-green-50 border border-green-200'
                    }`}>
                      <p className="text-sm text-center">
                        At current rate, budget will be exceeded in{' '}
                        <span className="font-bold">{analytics.dailyBurnRate.daysUntilBudgetExceeded} days</span>
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-indigo-50 rounded-lg p-4">
                ...
                <p className="text-sm text-indigo-700">
                  {analytics.spendingVelocity.velocityStatus === 'ON_TRACK'
                    ...
                    : analytics.spendingVelocity.velocityStatus === 'SLIGHTLY_HIGH'
                    ? `Try to keep daily spending under ${formatCurrency(analytics.spendingVelocity.recommendedDailySpending)} to stay on track.`
                    : `Consider reducing daily spending to ${formatCurrency(analytics.spendingVelocity.recommendedDailySpending)} to avoid overspending this month.`
                  }
                </p>
              </div>
            </>
