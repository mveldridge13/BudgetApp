  ...
  useEffect(() => {
    ...
    const initializeBiometric = async () => {
      try {
        ...
        console.log(
          '🔐 BiometricWrapper: Current lock state after init:',
          locked,
        );
        ...
        BiometricAuth.setOnLockStateChange(lockState => {
          if (isMounted) {
            console.log('🔐 BiometricWrapper: Lock state changed to:', lockState);
    ...
    try {
      ...
      const dataPromises = [
        TrendAPIService.getUserProfile()
          ...
          .catch(err =>
            console.warn('🔐 BiometricWrapper: Profile pre-load failed:', err),
          ),
        ...
        TrendAPIService.getTransactions()
          ...
          .catch(err =>
            console.warn(
              '🔐 BiometricWrapper: Transactions pre-load failed:',
              err,
            ),
          ),
        ...
        TrendAPIService.getCategories()
          ...
          .catch(err =>
            console.warn(
              '🔐 BiometricWrapper: Categories pre-load failed:',
              err,
            ),
          ),
      ];
